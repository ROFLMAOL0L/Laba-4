...
def handle_input(func, file_name, file_format, x0_input, x1_input, x_scaling_input=1):
    ...
    if 'x' not in func:
        ...
        x1 = x1_input
        dx = x_scaling_input
        while (x0%10 != 0) or (x1%10 != 0) or (dx%10 != 0):
            xmn = xmn*10
            x0 = x0*10
            x1 = x1*10
            dx = dx*10
        ...
        for x in x_range:
            ...
            # print(str(y) + ')')
        print('formula processing took', time() - handle_input_start_time, 'seconds')
        if file_format == 1:
            from excel_handler import deix
            deix(x_range, y_range, func, file_name)
        elif file_format == 2:
            from turtle_handler import deit
            deit(x_range, y_range, func, file_name)
        elif file_format == 3:
            from matplotlib_handler import deimpl
            deimpl(x_range, y_range, func, file_name)
